...
func _process(_delta: float) -> void:
	...
	if video_player.is_playing():
		...
		if texture:
			...
			if image:
				if not running_mode == MediaPipeTask.RUNNING_MODE_VIDEO:
					running_mode = MediaPipeTask.RUNNING_MODE_VIDEO
					_init_task()
				_process_video(image, Time.get_ticks_msec())
	# We have a strange bug (at least on V4L2/Pipewire driver of CameraFeed) that
	# activation/deactivation of camera feeds must be on the same thread, otherwise
	# it might block indefinitely, maybe the underlying implementation are wrong.
	if camera_feed:
		if camera_feed.feed_is_active != camera_feed_active:
			camera_feed.feed_is_active = camera_feed_active
...
func _select_camera() -> void:
	select_camera_dialog.popup_centered()
...
func _camera_added(id: int):
	...
	for feed in feeds:
		if feed.get_id() == id:
			opt_camera_feed.add_item(feed.get_name(), id)
