...
var task: MediaPipe.HandLandmarker
...
func init_task() -> void:
	...
	task = MediaPipe.HandLandmarker.new()
	task.initialize(base_options, running_mode, 1, 0.5, 0.5, 0.5)
...
func show_result(image: Image, result) -> void:
	for landmarks in result.get_hand_landmarks():
