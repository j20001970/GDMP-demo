...
func test_simple():
	var proto := MediaPipeProto.new()
	assert_true(proto.initialize("mediapipe.Color"))
	assert_true(proto.is_initialized())
	assert_eq(proto.get_type_name(), "mediapipe.Color")
	assert_true(proto.get_fields().has("r"))
	assert_true(proto.get_fields().has("g"))
	assert_true(proto.get_fields().has("b"))
